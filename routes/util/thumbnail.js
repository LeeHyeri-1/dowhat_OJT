...
function Thumbnail(rootOriginals, rootThumbnails, supportedImageTypes) {
    if (!rootOriginals) {
        throw new Error('provide rootOriginals');
    }
    if (!rootThumbnails) {
        throw new Error('provide rootThumbnails');
    }

    if (typeof rootOriginals !== 'string') {
        throw new TypeError('rootOriginals must be a string');
    }
    if (typeof rootThumbnails !== 'string') {
        throw new TypeError('rootThumbnails must be a string');
    }

    this.supportedImageTypes = supportedImageTypes || ['png', 'jpg', 'jpeg', 'gif'];

    if (!Array.isArray(this.supportedImageTypes)) {
        throw new TypeError('supportedImageTypes must be an array');
    }
 ...
 *                       just created or not.
 */
Thumbnail.prototype.ensureThumbnail = function ensureThumbnail(filename, thumbname, width, height, cb) {
    if (!filename) {
    ...
    if (filename !== path.basename(filename)) {
        throw new Error('filename contains a path');
    }

    var extension = path.extname(filename);
    if (!~this._supportedExtensions.indexOf(extension.toLowerCase())) {
    ...
    var thumbFilename = (thumbname != '' ? thumbname : path.basename(filename, extension) + '_thumb' + extension);
